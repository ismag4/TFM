...
def Gamma(phi, Q,f,M,Mpl, g, Cg, c,m):
    T = temperatura(phi, Q,f,M,Mpl, g)
...
def f_q(Q,c):
    return Q*(1+Q)**(c/2)/Q**(c/4)

def dfq(Q,c):
    return Q**(-c/4)*(1+Q)**(c/2)*(1-c/4+c*Q/(2*(1+Q)))
...
def Qphi(phi,f,M,Mpl,g, Cg, c,m):
    ...
    Q = newton(Fq,df,Q, c,tol=1e-9,iter=10000)
...
def loglike(f, logCg, M):
    ...
    try:
        ...
        chi2_ns = ((ns-0.9626)/0.0057)**2
        chi2_As = ((np.log(1e10*As) - 3.044)/0.014)**2

        if r > 0.036:
            chi2_r = ((r-0.036)/0.022)**2
        elif r < 0:
            return -np.inf
        else:
            chi2_r = (r/0.022)**2
